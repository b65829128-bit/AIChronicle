...
- **类大小控制**：单个类文件原则上不超过 ~1000 行。接近时按领域拆成 partial（如 `ToolExecutor.*.cs`）或拆出独立类。发现已有大文件被持续追加时，主动提出拆分方案
- **依赖引入需审批**：原则上不引入新第三方库（当前 NuGet 仅 NAudio）。若某功能用第三方库可显著降低维护成本（如 JSON 解析、正则、TTS 引擎、HTTP 库），**先向用户说明引入理由、备选方案、体积/许可影响，经明确同意后才添加**——第三方库是"维护成本换运行时成本"，权衡权在开发者
  - **兼容性风险（必须权衡）**：新库可能与其他模组冲突——同名/同版本 DLL 覆盖、依赖的原生库版本不一致、与其他模组的共享库（如 Newtonsoft.Json、全模组常用 HTTP 库）版本冲突。引入前需评估：该库是否已被其他常见模组使用、能否只引用不打包（交给宿主 Mod 提供）、冲突时是否有降级路径（如回退到自带实现）。本模组自身依赖四前置（Harmony/ButterLib/UIExtenderEx/MCM），引入新库时要考虑与它们的版本兼容。权衡权始终在开发者
- **新功能可维护性**：加入新功能时遵循——
  - 遵循现有分层（Core/Agents/Entities/Tools/UI/Systems），不绕开既有模式另起炉灶
  - 复用现有基础设施（SafeFileIO / MainThreadExecutor / AgentScheduler 事件队列 / DebugLogger），不重复造轮子
  - 工具、事件、补丁等新入口必须登记到对应清单（tools.json、调度器 switch、SubModule 注册），并同步更新文档
  - 关注增量成本：新增的每一条路径（事件类型/工具/激活时机）都会带来长期维护与 token 消耗，能复用优先复用
...
- ~~NPC 自立建国~~ → `create_kingdom`（已实现，**实验性功能：未经实机测试，为未来功能提前做的准备**）<br>
  ...
  - **封臣叛乱先行**：`ApplyByCreateKingdom` 内部 `ClanLeaveKingdom` 只置 Kingdom=null（不转封地、不宣战）——封臣直接建国会"无声脱离旧国"。故工具内先 `ApplyByLeaveWithRebellionAgainstKingdom`（保封地、对旧国及其交战方宣战）再建国。叛乱触发 `war_declared` 入史。
