...
- 原始史料以 JSONL 格式存储在 `NPCs/World/history/events_{年份}.txt`，永久保存。**固定写入当前战役目录**（实时解析战役路径，不缓存），避免进档初期被写入基础目录 `Prompts/NPCs/World/history/` 的时序问题
...
- 氏族领袖、国王、玩家死亡时，史官自动编纂**列传/本纪**（人物传记）。**身份在死亡前一刻捕获**（`BeforeHeroKilled`，先于原版继任改选），关闭「所有贵族立传」后国王/族长仍会立传。**传记精准定位**：`query_character` 支持**重名消歧**（多个同名者时列出全部候选人：编号/氏族/王国/年龄）与**按编号精确查询**（`CharacterObject_XXXX`）；传记事件携带死者编号，提示词要求史官用编号精查——修复了重名人物导致传记张冠李戴（氏族、生卒年全错）的 bug
...
- **玩家不可用**：建国是重大国体变更，玩家走原版正规流程（与总督对话建国）；`create_kingdom` 仅限 NPC 封臣/独立氏族领袖使用，秘书处（M 键）已排除该工具，工具内部亦拒绝玩家（MainHero）调用
...
- Agent 可以调用 `query_party_troops` 查看部队详情。**军情迷雾**：自己与同阵营部队全量（金币、日薪、兵力/伤兵/上限、各兵种数量经验升级路径、俘虏可招募性、物品栏、装备栏）；异国部队仅侦察估计——近距给兵力带、**规模上限**与兵种构成（约 ±20%），远距给宽泛区间与定性描述（含规模上限区间），跨海/远处只有传闻，不泄露军饷、经验、装备等机密。**军团上下文**：全量报告附军团段（该部队是否隶属军团、军团长是谁、全军合计兵力、成员名单——自己身处军团时不会再误以为只带本部人马）；近距/远方侦察亦附军团隶属（军团旗帜地图可见）
...
- **原版逻辑**：无王国的独立家族（灭国幸存者、自愿离国者、丢光封地者）若 28 天不加入势力，会被 `FactionDiscontinuationCampaignBehavior` 自动灭族（仅剩百科）
- **本模组默认 ban 掉此逻辑**（MCM「独立家族永不失踪」，默认开启）：独立家族永久保留，作为封臣自省、玩家劝降、密使交涉的对象，不会被原版计时器抹掉
- 关闭开关则恢复原版 28 天灭族行为
...
**场景连接配置**（懒人可只填上面兜底三件套，各场景留空即全用兜底；每个场景额外有一个「接口类型」下拉，默认「跟随兜底」，可单独覆盖——例如史官用 DeepSeek 保文笔、其余用便宜模型）：

| 场景组 | 设置项 | 说明 |
|--------|--------|------|
| 对话与书信场景 | URL / 模型 / API 密钥 / 接口类型 | 玩家对话（【AI 聊天】）与写信共用（同一线程）；留空回退兜底 |
| 政务外交场景 | URL / 模型 / API 密钥 / 接口类型 | 国王内外政务审视（KingDiplomacy）；留空回退兜底 |
| 外交问询场景 | URL / 模型 / API 密钥 / 接口类型 | 国王遣使问询他国（KingConsult）；留空回退兜底 |
| 封地审视场景 | URL / 模型 / API 密钥 / 接口类型 | 被夺封方审视处境（FiefReview）；留空回退兜底 |
| 封臣谏言场景 | URL / 模型 / API 密钥 / 接口类型 | 封臣自省（self_review）/ 密使回应（envoy_reply）；留空回退兜底 |
| 天意建族场景 | URL / 模型 / API 密钥 / 接口类型 | 家族补充「天意」（clan_replenishment）；留空回退兜底 |
| 史官场景 | URL / 模型 / API 密钥 / 接口类型 | 编年史/列传/专题史（historian）；留空回退兜底 |
| 记忆巩固场景 | URL / 模型 / API 密钥 / 接口类型 | 记忆巩固 pass（consolidation）；留空回退兜底 |
| 签到场景 | URL / 模型 / API 密钥 / 接口类型 | 行为/计划签到（chat）；留空回退兜底 |
| 秘书处场景 | URL / 模型 / API 密钥 / 接口类型 | 玩家秘书处（chancery，M 键）；留空回退兜底 |

> 每个场景组各有「测试此场景」按钮，用本场景生效配置（留空字段回退到兜底）测试连通性与 function calling 支持。**逐字段兜底**：哪个字段留空就用兜底的哪个字段——例如只给史官配高规格模型，URL 和密钥沿用兜底即可；接口类型同样可逐场景覆盖（默认「跟随兜底」）。
| 双倍声望 | 战斗中声望翻倍 | 关闭 |
...
| 独立家族永不失踪 | 开启后无王国的独立家族（灭国幸存者/自愿离国者/丢光封地者）不再被原版 28 天计时器自动灭族，永久保留，契合封臣自省与玩家劝降；关闭恢复原版（28 天不加入势力即灭族，仅剩百科） | 开启 |
...
- **修复秘书处上下文污染（M 键幻觉）**：秘书处打开时双侧实体都是玩家（`agentId == targetId`），`ContextBuilder` 的「审视王国处境/自省」语义优先于秘书处分支，导致秘书处被误导成"带兵的王国决策者"（如回答"扫描周围环境"却幻觉成"已向女皇呈递谏言""我部约七十众"）。修复后秘书处专用文本优先：自我状态不再注入玩家部队/领地信息，目标认知/关系/目标/客观关系全部采用「行政助手—主人」语义

### v2.2.1 更新要点

- **「皇帝」语义澄清（初始历史）**：总史与帝国纪事在「皇帝」称号首次出现处点破其军中来源——「此名号本出军中，不过是战时统军的名分，战罢即须交还兵权，非后世坐拥天下者比」。消除中文读者对"皇帝为何交权"的困惑（对应罗马 `imperator`=统帅的语义，与后文「不再交还权柄」形成演变弧线）
- **文档新增中英双语（i18n）长线待办**：目标中文→中英双语，架构方案见 AGENTS.md「扩展方向（未来）」
...
- **中英双语（i18n，长线待办）**：当前为完全中文体验（提示词/史书/UI/语义设计）。目标支持**中英双语**——提示词按语言分目录、MCM「界面语言」设置（Auto 跟随游戏）、LLM 输出跟随玩家语言、史官文风随语言切换（中文资治通鉴风 / 英文编年史风）。架构方案见 AGENTS.md「扩展方向（未来）」。个人能力所限不做全语言全球化，未来模组有热度后靠社区维护。
