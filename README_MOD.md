...
| 启用家族补充 | 封臣/雇佣兵家族低于阈值时激活「天意」补充新家族（**实验性，默认关闭**） | 关闭 |
...
- [ ] **环境扫描工具改用地图比例**：`query_surroundings` 的扫描半径目前使用 km 单位，不符合游戏实际情况——应改为占地图的比例（类似 `query_party_troops` 的「情报侦察半径」做法），并同步调整 MCM「环境扫描半径（km）」设置项
- [ ] **城堡被围攻状态**：查询城堡的工具（`query_settlement` 等）应增加一项状态——城堡当前**是否正在被围攻**，让 Agent 能感知围城动态（配合「禁止原版外交」下 AI 围城决策）
